...
/*        if(role.getName().toUpperCase().equals(Role.ADMIN)) {
    ...
    public String login(String phoneNumber, String password, Long roleId) throws Exception {
        ...
        //ly do tat vi form dart khong truyen roleId
...
//            throw new DataNotFoundException(localizationUtils.getLocalizedMessage(MessageKeys.ROLE_DOES_NOT_EXISTS));
        ...
        authenticationManager.authenticate(authenticationToken); //kiểm tra tk mk nếu đúng thỉ trả ra token
        //Spring Security sử dụng một cơ chế gọi là "UserDetailsService" để tìm kiếm thông tin người dùng từ cơ sở dữ liệu hoặc bất kỳ nguồn dữ liệu nào khác.
        //Sau khi nhận được thông tin người dùng từ UserDetailsService, Spring Security sẽ so sánh mật khẩu đã được mã hóa trong UserDetails với mật khẩu đã được cung cấp trong authenticationToken
        //Đây là một quá trình so sánh an toàn, trong đó mật khẩu được mã hóa lại và so sánh với mật khẩu đã được mã hóa trong cơ sở dữ liệu.
